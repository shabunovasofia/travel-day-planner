 ...
 */
@Component
@ConditionalOnProperty(prefix = "planner.integration", name = "enabled", havingValue = "true", matchIfMissing = true)
public class PlannerUpstreamClient {
	...
	public PlannerUpstreamClient(PlannerIntegrationProperties properties) {
		this.locationContextClient = RestClient.builder()
				.baseUrl(trimSlash(properties.getLocationContextBaseUrl()))
				.build();
		this.placesClient = RestClient.builder()
				.baseUrl(trimSlash(properties.getPlacesBaseUrl()))
				.build();
	}

	public LocationContextAnalyzeResponse analyzeLocation(
			String location, LocalTime startTime, LocalTime endTime, WalkPace pace) throws RestClientException {
		...
		body.setPace(pace);
		return locationContextClient.post()
	...
	public List<PlaceDto> searchPlaces(
			...
			List<String> categories) throws RestClientException {
		...
		PlacesSearchApiResponse response = placesClient.post()
	...
	public static WalkPace parseWalkPace(String raw) {
		...
		try {
			...
			throw new IllegalArgumentException("Некорректный темп прогулки: ожидается SLOW, MEDIUM или FAST.", e);
		}
	}
