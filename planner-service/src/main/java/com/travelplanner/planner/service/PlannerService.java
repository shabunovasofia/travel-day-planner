...
public class PlannerService {
	...
	public PlanResponse build(PlanRequest request) {
		...
		RoutePlanning.OptimizedSchedule optimized =
				RoutePlanning.optimizeOrdering(places, dayStart, dayEnd);
	...
	private List<PlaceDto> resolvePlaces(
		...
		if (!integrationProperties.isEnabled() || client == null) {
			warnings.add("Интеграция с location-context/places отключена — используются только места из запроса.");
			return merged;
		}
		try {
			WalkPace pace = PlannerUpstreamClient.parseWalkPace(request.getWalkPace());
			LocationContextAnalyzeResponse ctx = client.analyzeLocation(walkLocation, dayStart, dayEnd, pace);
			List<String> categories = PlannerUpstreamClient.nonEmptyCategories(request.getDiscoverCategories());
			if (categories.isEmpty()) {
				warnings.add("Указан walkLocation, но discoverCategories пуст — внешний поиск мест не выполнялся.");
				return merged;
			}
			List<PlaceDto> discovered = client.searchPlaces(
					ctx.getLatitude(),
					ctx.getLongitude(),
					ctx.getRadiusMeters(),
	...
	private void validateRequest(PlanRequest request) {
		...
		boolean canDiscover = request.getWalkLocation() != null && !request.getWalkLocation().isBlank()
