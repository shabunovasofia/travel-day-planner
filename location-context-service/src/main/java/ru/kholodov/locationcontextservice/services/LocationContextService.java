 ...
 *
 * <p>Внутренне выполняет: геокодирование → изохрону → поиск мест → планирование, но возвращает
 * клиенту только финальный план посещения.
 *
...
public class LocationContextService {
  ...
  public PlanBuildResponse.PlanData buildRoute(LocationContextRequest request) {
    ...
    log.info(
        "Построен маршрут: {} мест, общее время: {} ч",
        planData.getTotalPlaces(),
        planData.getTotalHours());
  ...
  private int calculateRadius(Coordinates center, double availableHours, Pace pace) {
    return isochroneService
        .calculateRadius(center, availableHours, pace)
