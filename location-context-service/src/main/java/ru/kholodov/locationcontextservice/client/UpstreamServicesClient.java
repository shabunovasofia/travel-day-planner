 ...
 * Транзиентные ошибки upstream-сервисов обрабатываются Resilience4j (retry + circuit breaker). Для
 * {@link #searchPlaces} при исчерпании попыток / открытом CB возвращается пустой список; для {@link
 * #buildPlan} ошибка пробрасывается выше (план без planner-сервиса не построить).
 */
@Slf4j
@Component
public class UpstreamServicesClient {
  ...
  /**
   * Собирает два {@link RestClient}'а — для places-service и planner-service — с baseUrl из {@link
   * IntegrationProperties}. Таймауты и перехватчики берутся из общего билдера, который
   ...
   */
  public UpstreamServicesClient(RestClient.Builder restClientBuilder, IntegrationProperties props) {

    this.placesClient = restClientBuilder.baseUrl(trimSlash(props.getPlacesBaseUrl())).build();

    this.plannerClient = restClientBuilder.baseUrl(trimSlash(props.getPlannerBaseUrl())).build();
  }
  ...
  /**
   * Срезает финальный {@code /} у URL, если он есть. Защита от двойных слэшей в путях. При пустом
   * URL подставляется дефолт на localhost — это compile-time fallback, в проде {@link
   * IntegrationProperties} требует {@code @NotBlank}.
   *
