 ...
 *
 * <p>Перехватывает основные типы исключений и формирует единообразный JSON-ответ {@link ApiErrors}
 * с массивом {@code errors} (каждая ошибка — code + message). HTTP-статусы:
 *
 * <ul>
...
public class GlobalExceptionHandler {
  ...
  /**
   * Обрабатывает ошибки валидации входных данных: каждое поле — отдельный элемент массива {@code
   * errors}, что позволяет клиенту разобрать ошибки по полям без парсинга строки.
   *
  ...
  public ResponseEntity<ApiErrors> handleValidation(MethodArgumentNotValidException ex) {
    List<ApiErrors.ApiError> errors =
        ex.getBindingResult().getFieldErrors().stream()
            .map(
                fe ->
                    new ApiErrors.ApiError(
                        CODE_VALIDATION, fe.getField() + ": " + fe.getDefaultMessage()))
            .toList();
    if (errors.isEmpty()) {
   ...
   *
   * @param ex {@link ResourceAccessException}, обычно содержит {@code
   ...
   *
   * <p>Стектрейс целиком уходит в логи, наружу — обобщённое сообщение без внутренних деталей, чтобы
   * не утекли сведения о реализации.
   *
   * @param ex исключение
