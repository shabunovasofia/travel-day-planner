 ...
 *
 * <p>Содержит уточнённые координаты, рассчитанный радиус поиска, доступное время и исходные
 * параметры запроса.
 *
 * <p>Объект является неизменяемым (record), создаётся через фабричный метод {@link #create(String,
 * double, double, LocalTime, LocalTime, Pace)}.
 *
 ...
 * @param availableHours количество доступных часов (разница между {@code endTime} и {@code
 *     startTime})
...
public record LocationContextResponse(
  ...
  /**
   * Фабричный метод для создания ответа с автоматическим расчётом радиуса и доступного времени.
   *
  ...
  public static LocationContextResponse create(
    ...
    return new LocationContextResponse(
        resolvedLocation, latitude, longitude, radius, availableHours, startTime, endTime, pace);
  }
}
