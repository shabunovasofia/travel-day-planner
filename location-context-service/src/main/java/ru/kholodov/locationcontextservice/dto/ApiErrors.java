 ...
 *
 * <p>Формат позволяет вернуть несколько ошибок за один ответ — например, при валидации
 * сразу нескольких полей запроса. Каждая ошибка имеет машинно-читаемый {@link ApiError#code}
 * и человекочитаемое {@link ApiError#message}.
 *
...
public record ApiErrors(
        @Schema(description = "Список ошибок (всегда непустой)")
     ...
     *
     * @param code машинно-читаемый код (например, {@code VALIDATION_ERROR}, {@code ADDRESS_NOT_FOUND})
     * @param message человекочитаемое сообщение
     */
