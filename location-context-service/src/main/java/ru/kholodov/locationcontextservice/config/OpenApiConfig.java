 ...
 *
 * <p>Задаёт метаданные API (название, описание, версия, контактные данные), которые
 * отображаются в Swagger UI ({@code /swagger-ui.html}) и в JSON-схеме ({@code /v3/api-docs}).
 *
    ...
    public OpenAPI locationContextOpenApi() {
        return new OpenAPI()
                .info(new Info()
                        ...
                        .version("0.0.1")
                        .contact(new Contact()
                                .name("Stepan Kholodov")
                                .email("kholodov_stepan@inbox.ru")));
    }
}
